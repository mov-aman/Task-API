...
def test_create_task():
    r = requests.post('http://localhost:8000/v1/tasks', json={"title": "My First Task","is_completed": False})
...
def test_update_task():
    r = requests.put('http://localhost:8000/v1/tasks/6', json={"title": "My 1st Task", "is_completed": True})
    assert not r.content

def test_delete_task():
    r = requests.delete('http://localhost:8000/v1/tasks/5')
    assert not r.content
